...
library(ggplot2)
test_flow=NULL
# Install and load rstudioapi if not installed
if (!requireNamespace("rstudioapi", quietly = TRUE)) {
  install.packages("rstudioapi")
}

# Get the directory of the currently opened script and set it as working directory
current_path <- rstudioapi::getActiveDocumentContext()$path
setwd(paste0(dirname(current_path),"/hgs_onezone"))
...
list=dir(pattern = "lys_Eo.nodal_fluid_mass_balance.id_str.dat")
# 1. Read all lines
...
#MULTIPLE
setwd(paste0(dirname(current_path),"/hgs_multi_from_one_shp"))
list=dir(pattern = "lys_Eo.nodal_fluid_mass_balance.*")
list <- grep("_selection", list, value = TRUE, invert = TRUE)
list
